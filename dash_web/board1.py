...
def create_app1():
    ...
    def update_image(feature, transformation):
        try:
            buf = create_figure(feature, transformation)
            image_data = encode_image(buf)
            return html.Img(src='data:image/png;base64,{}'.format(image_data), style={'width': '100%', 'height': 'auto'})
        except Exception as e:
            print(f"Error updating image: {e}")
            return html.P("Error occurred while updating the image.")
